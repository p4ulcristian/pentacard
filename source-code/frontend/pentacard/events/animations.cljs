...
(defn callback-x [^js ref ^js spring-value callback-id db-after-animation] 
  (if-not (animation-finished? spring-value)
    ...
    (do (dispatch [:render/remove-callback callback-id])
        (dispatch [:db/set-db db-after-animation])
        (println "Animation finished"))))


(defn animate-position! [ref db-after-animation]
 (let [x-callback-id (str (random-uuid)) 
   ...
   (dispatch [:render/add-callback 
              x-callback-id
              #(callback-x ref x-spring x-callback-id db-after-animation)])))

(reg-event-db
 :animation/move-card!
 (fn [db [_ {:keys [ref-key db-after-animation]}]]
   (let [card-ref (get-in db [:refs ref-key])]
     (animate-position! card-ref db-after-animation)
     db-after-animation
     db)))
